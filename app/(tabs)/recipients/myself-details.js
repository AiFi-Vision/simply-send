...
import { Text, Button, Switch } from "@/elements";
import ContainerView from "@/components/ContainerView";
import { getAvatarName } from "@/helper";
import { COLORS } from "@/styles/colors";
import globalStyles from "@/styles/global";

const MySelfDetailScreen = () => {
  const navigation = useNavigation();
  const router = useRoute();
  const { info } = router.params;
  const [nickNameShow, setNickNameShow] = useState(false);
  const [isOwnSwitch, setIsOwnSwitch] = useState(true);
  const [isPrimarySwitch, setIsPrimarySwitch] = useState(false);
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.item}>
            <Text style={globalStyles.h4}>Recipient type</Text>
            <Text style={styles.itemContent}>Private</Text>
          </View>
          <View style={styles.item}>
            <Text style={globalStyles.h4}>ACH routing number</Text>
            <Text style={styles.itemContent}>{info.routingNumber}</Text>
          </View>
          <View style={styles.item}>
          ...
          <View style={styles.item}>
            <Text style={globalStyles.h4}>Account type</Text>
            <Text style={styles.itemContent}>{info.accountType}</Text>
          </View>
          <View style={styles.item}>
            <Text style={globalStyles.h4}>Email</Text>
            <Text style={styles.itemContent}>wilsonjenny12@gmail.com</Text>
          </View>
          <View style={styles.item}>
            <Text style={globalStyles.h4}>Address</Text>
            <Text style={styles.itemContent}>
              {info.address}, {info.city} {info.postCode}
            </Text>
          </View>
          <View style={styles.switchItem}>
            <Text style={styles.itemContent}>I own this account</Text>
            <Switch value={isOwnSwitch} onChange={setIsOwnSwitch} />
          </View>
          <View style={styles.switchItem}>
            <Text style={styles.itemContent}>
              Primary account to receive USD
            </Text>
            <Switch value={isPrimarySwitch} onChange={setIsPrimarySwitch} />
          </View>
          <Text style={[globalStyles.h4, { marginVertical: 20 }]}>
            Set this account as your primary USD account so others can send you
            USD without needing to know your bank details.
          </Text>
        </View>
      </View>
    </ContainerView>
...
export default MySelfDetailScreen;
